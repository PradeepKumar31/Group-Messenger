...
public class GroupMessengerProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        try {
            ...
            String content = (String) values.get("value");
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        try {
            ...
            String values = new String(content).substring(0, size);

            MatrixCursor.RowBuilder row = matrixCursor.newRow();
            row.add(columns[0], selection);
            row.add(columns[1], values);
            inputStream.close();
        }
        catch (Exception e){
            Log.e(TAG, "Exception while adding new row");
        }
        return matrixCursor;
    }
}
